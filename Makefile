...
READYSHELL_DIR = $(APPS_DIR)/readyshell
...
READYSHELL_CAT_DEBUG ?= 0
READYSHELL_CAT_DEBUG_CFLAG = $(if $(filter 1,$(READYSHELL_CAT_DEBUG)),-DDEBUG_RSCAT=1,)
...
READYSHELL_EASYFLASH_CCFLAGS = $(READYSHELL_CCFLAGS) -DRS_C64_OVERLAY_PRELOADED=1
...
READYSHELL_OVL7_CFLAGS = $(READYSHELL_CCFLAGS) $(READYSHELL_CAT_DEBUG_CFLAG) --code-name OVERLAY7 --rodata-name OVERLAY7 --bss-name OVERLAY7
READYSHELL_OVL8_CFLAGS = $(READYSHELL_CCFLAGS) --code-name OVERLAY8 --rodata-name OVERLAY8 --bss-name OVERLAY8
...
READYSHELL_OVL6_DISK = $(OBJ_DIR)/rsfops.prg
READYSHELL_OVL7_DISK = $(OBJ_DIR)/rscat.prg
READYSHELL_OVL8_DISK = $(OBJ_DIR)/rscopy.prg
...
READYSHELL_OVERLAY6_SRCS = \
	$(READYSHELL_CORE_DIR)/rs_cmd_delren_c64.c \
	$(READYSHELL_CORE_DIR)/rs_cmd_putadd_c64.c
READYSHELL_OVERLAY7_SRCS = \
	$(READYSHELL_CORE_DIR)/rs_cmd_cat_c64.c
READYSHELL_OVERLAY8_SRCS = \
	$(READYSHELL_CORE_DIR)/rs_cmd_copy_c64.c
...
READYSHELL_OVERLAY8_OBJS = $(patsubst %.c,$(READYSHELL_OBJ_DIR)/overlay8/%.o,$(READYSHELL_OVERLAY8_SRCS))
...
PROGRAMS = $(BOOT) $(PREBOOT) $(SETD71) $(SHOWCFG) $(TEST_REU) $(LAUNCHER) $(EDITOR) $(QUICKNOTES) $(CALCPLUS) $(HEXVIEW) $(CLIPMGR) $(REUVIEWER) $(SYSINFO) $(TASKLIST) $(SIMPLEFILES) $(SIMPLECELLS) $(GAME2048) $(DEMINER) $(SIDETRIS) $(CAL26) $(DIZZY) $(READMEAPP) $(READYSHELL)
...
# System Info app (loads at $1000)
$(SYSINFO): $(APPS_DIR)/sysinfo/sysinfo.c $(APPS_DIR)/sysinfo/sysinfo_uci.c $(APPS_DIR)/sysinfo/sysinfo_uci_asm.s $(LIB_SYSINFO)
	$(CC) $(APP_CFLAGS) -Os -m $(OBJ_DIR)/sysinfo.map -o $@ $^
...
$(READYSHELL_EASYFLASH_OBJ_DIR)/resident/%.o: %.c
	@mkdir -p "$(dir $@)"
	$(CC) $(READYSHELL_EASYFLASH_CCFLAGS) -c -o $@ $<
...
$(READYSHELL_OBJ_DIR)/overlay8/%.o: %.c
	@mkdir -p "$(dir $@)"
	$(CC) $(READYSHELL_OVL8_CFLAGS) -c -o $@ $<

# ReadyShell app (loads at $1000) + overlay sidecars
$(READYSHELL): $(READYSHELL_RESIDENT_OBJS) $(READYSHELL_OVERLAY1_OBJS) $(READYSHELL_OVERLAY2_OBJS) $(READYSHELL_OVERLAY3_OBJS) $(READYSHELL_OVERLAY4_OBJS) $(READYSHELL_OVERLAY5_OBJS) $(READYSHELL_OVERLAY6_OBJS) $(READYSHELL_OVERLAY7_OBJS) $(READYSHELL_OVERLAY8_OBJS) $(CFG_DIR)/ready_app_overlay.cfg
	$(CC) -t c64 -C $(CFG_DIR)/ready_app_overlay.cfg \
		...
		$(READYSHELL_RESIDENT_OBJS) $(READYSHELL_OVERLAY1_OBJS) $(READYSHELL_OVERLAY2_OBJS) $(READYSHELL_OVERLAY3_OBJS) $(READYSHELL_OVERLAY4_OBJS) $(READYSHELL_OVERLAY5_OBJS) $(READYSHELL_OVERLAY6_OBJS) $(READYSHELL_OVERLAY7_OBJS) $(READYSHELL_OVERLAY8_OBJS)
...
easyflash-verify: easyflash
	...
	$(PYTHON) $(BUILD_SUPPORT_DIR)/verify_readyos_shim.py --check-easyflash-bin; \
...
clean:
	...
	rm -f $(READYSHELL_OVL1_PRG) $(READYSHELL_OVL2_PRG) $(READYSHELL_OVL3_PRG) \
		...
		$(READYSHELL_OVL7_PRG) $(READYSHELL_OVL8_PRG) \
	...
	rm -f $(READYSHELL_OVL1_DISK) $(READYSHELL_OVL2_DISK) $(READYSHELL_OVL3_DISK) \
		...
		$(READYSHELL_OVL7_DISK) $(READYSHELL_OVL8_DISK)
	rm -f *.d64
	rm -f *.d71
	rm -f *.d81
	rm -f *.crt
...
help:
	...
	@echo "  shim-verify - Verify the shared shim stays byte-identical to baseline"
	...
	@echo "  $(SYSINFO)  - System information viewer (loads at \$$1000)"
