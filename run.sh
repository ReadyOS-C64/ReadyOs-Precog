...
#define READYOS_BOOT_VERSION_TEXT "PRECOG V${RUN_VERSION_TEXT}"
...
msg_version:
    .byte "PRECOG V${RUN_VERSION_TEXT}"
msg_version_end:
EOF
}
...
maybe_build() {
    ...
    if ! make -B BUILD_SUPPORT_DIR="$BUILD_SUPPORT_DIR" "$DISK_FILE_1" "$DISK_FILE_2"; then
