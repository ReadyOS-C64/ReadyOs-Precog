...
/*
 * DEBUG_RSCAT is a build-time switch for the paging probe version of CAT.
 * The checked-in default is READYSHELL_CAT_DEBUG=1 so plain run.sh builds the
 * current debug-enabled CAT variant. Override with READYSHELL_CAT_DEBUG=0 to
 * compile out DBG:BEGIN / DBG:PARAM / DBG:END and emit only payload lines.
 */
